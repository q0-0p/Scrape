...
for (var abc = 1; abc < 9; abc++) {
    var htmlString = fs.readFileSync('allPages' + abc + '.txt').toString();
...
for (var xyz = 0; xyz < links.length; xyz++) {
    if (links[xyz]==='n/a'){
        fs.writeFileSync('numemLinks.txt', '--no link found'+ '\n', { flag: 'a' });
        
    }
    else {
        fs.writeFileSync('numemLinks.txt', 'http://www.cra-arc.gc.ca' + links[xyz] + '\n', { flag: 'a' });
    }
}