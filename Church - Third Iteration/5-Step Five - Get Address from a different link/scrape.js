...
var w = fs.read('names.txt');
urls = w.split('\n');
for(var g= 0;g< urls.length; g++){
    urls[g] = "https://chimp.net/charities/"+ urls[g];
}
    ...
    return function (status) {
        ...
        if (status === "success") {
            //this means its past all the links and reached the end of the links array
                ...
                if(x<2000)
                fs.write('rawHTML1.txt', page.content, 'a');
                else if(x>=2000&& x<4000)
                fs.write('rawHTML2.txt', page.content, 'a');
                else if(x>=4000&& x<6000)
                fs.write('rawHTML3.txt', page.content, 'a');
                else 
                fs.write('rawHTML4.txt', page.content, 'a');
