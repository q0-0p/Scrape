...
var urls = [];
for(var x = 0; x<JSONarr.length; x++){
    if(JSONarr[x].hasOwnProperty('url2')){
        urls.push(JSONarr[x].url2);
    }
}
    ...
    var genericCallback = function () {
        return function (status) {
            ...
            if (status === 'fail') {
                ...
                console.log("THERE")
                fs.write('revampedBAD.txt', urls[x] + ' index number: ' + x + ' Province: ' +  + '\n', 'a');
                x++;
            ...
            if (status === "success") {
                ...
                if (urls[x] === 'undefined') {
                    ...
                    fs.write( 'revamped.txt', page.content, 'a');
